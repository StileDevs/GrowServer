...
export function handleBlockPlacing(p: Arg): boolean {
  ...
  switch (p.actionType) {
    ...
    case ActionTypes.LOCK: {
      ...
      if (mLock) {
        if (p.block.lock) {
          p.peer.send(Variant.from("OnTalkBubble", p.peer.data.netID, "This area is `4already locked``", 0, 1));
          return false;
        }

        if (typeof p.world.data.owner?.id === "number" && p.world.data.owner.id !== p.peer.data?.id_user) {
          p.peer.send(Variant.from("OnTalkBubble", p.peer.data.netID, "The tile owner `2allows`` public building but `4not`` for this specific block.", 0, 1));
          return false;
        }
      ...
      if (p.world.data.blocks?.find((b) => b.lock?.ownerUserID && b.lock.ownerUserID !== p.peer.data?.id_user)) {
        p.peer.send(Variant.from("OnTalkBubble", p.peer.data.netID, `Can't put lock, there's other locks around here.`, 0, 1));
        return false;
      }

      if (p.block.x === 0 && p.block.y === 0) {
        p.peer.send(Variant.from("OnTalkBubble", p.peer.data.netID, "You `4cannot`` place locks over here!", 0, 1));
        return false;
      }
      ...
      p.peer.everyPeer((pa) => {
        if (pa.data?.world === p.peer.data?.world && pa.data?.world !== "EXIT")
          pa.send(
            Variant.from("OnTalkBubble", p.peer.data.netID, `\`3[\`w${p.world.worldName} \`ohas been World Locked by ${p.peer.name}\`3]`),
            Variant.from("OnConsoleMessage", `\`3[\`w${p.world.worldName} \`ohas been World Locked by ${p.peer.name}\`3]`),
...
export function tileUpdate(base: BaseServer, peer: Peer, actionType: number, block: Block, world: World): void {
  peer.everyPeer((p) => {
...
export function tileVisualUpdate(peer: Peer, block: Block, visualFlags: number, everyPeer = false): void {
