...
export class Input {
  ...
  public async execute(action: NonEmptyObject<{ text: string }>): Promise<void> {
    try {
      ...
      if (text.startsWith("/")) {
        ...
        if (!cmdCd)
          this.base.cache.cooldown.set(`${commandName}-netID-${this.peer.data?.netID}`, {
            limit: 1,
            time: Date.now()
          });
        else {
        ...
        setTimeout(() => {
          this.base.cache.cooldown.delete(`${commandName}-netID-${this.peer.data?.netID}`);
      ...
      this.peer.every((p) => {
        if (p.data?.world === this.peer.data?.world && this.peer.data?.world !== "EXIT") {
          p.send(Variant.from("OnTalkBubble", this.peer.data?.netID || 0, action.text, 0), Variant.from("OnConsoleMessage", `CP:0_PL:0_OID:_CT:[W]_ <\`w${this.peer.data?.tankIDName}\`\`> ${action.text}`));
        }
      });
    } catch (e) {
      consola.warn(e);
      this.peer.send(Variant.from("OnConsoleMessage", "`4Unknown command.`` Enter `$/help`` for a list of valid commands"));
    }
  }
}
