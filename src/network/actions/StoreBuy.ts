...
export class StoreBuy {
  ...
  private addStoreItems(dialog: DialogBuilder, category: string): DialogBuilder {
  ...
  public async execute(action: NonEmptyObject<{ action: string; itemID: string; item: string }>): Promise<void> {
    const validTabs = ["main", "player", "locks", "itempacks", "creativity", "token"];
