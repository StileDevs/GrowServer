...
export class DropEnd {
  ...
  public async execute(): Promise<void> {
    if (!/\d/.test(this.action.drop_count) || !/\d/.test(this.action.itemID)) {
      this.peer.send(Variant.from("OnTalkBubble", this.peer.data.netID, "Uh oh, thats not a valid number"));
      return;
    }
    ...
    if (!itemExist || itemExist.amount <= 0) {
      this.peer.send(Variant.from("OnTalkBubble", this.peer.data.netID, "That item, seems not exist in your inventory"));
      return;
    }
    ...
    if (count <= 0) {
      this.peer.send(Variant.from("OnTalkBubble", this.peer.data.netID, "Nice try. You remind me of myself at that age."));
      return;
    }
