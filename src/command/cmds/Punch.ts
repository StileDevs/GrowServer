...
export default class PunchID extends Command {
  ...
  public async execute(): Promise<void> {
    ...
    if (!newPunchID || isNaN(Number(newPunchID))) {
      this.peer.send(Variant.from("OnConsoleMessage", "`4Usage: /pid <new_id> (must be a number)"));
      return;
    }

    this.peer.customPunchID = Number(newPunchID);

    this.peer.send(Variant.from("OnConsoleMessage", `\`2Your Punch ID has been changed to: ${newPunchID}`));
