...
export class TemplateParser {
  ...
  public splice(t: Template) {
    const ingredients = t.parameters
      .slice(0, 2)
      .map((ingredient) => String(ingredient.value));
    const ingredient_ids = ingredients.map((ingredient) =>
      this.itemIdFromName(ingredient),
    );
  ...
  public item(t: Template) {
    ...
    const chi = this._chi.includes(t.parameters[1]?.value.toLowerCase())
      ? t.parameters[1]?.value.toLowerCase()
      : "";
    return [desc, chi];
  }
