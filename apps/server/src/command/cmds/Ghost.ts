...
export default class Ghost extends Command {
  ...
  public async execute(): Promise<void> {
    ...
    if (this.peer.data.state.isGhost) {
      ...
      this.peer.send(Variant.from("OnConsoleMessage", "`2Ghost mode enabled``. You can now walk through blocks!"));
        ...
        world.every((p) => {
          p.send(
            Variant.from(
              ...
              "OnSetClothing",
              [this.peer.data.clothing.hair, this.peer.data.clothing.shirt, this.peer.data.clothing.pants],
              [this.peer.data.clothing.feet, this.peer.data.clothing.face, this.peer.data.clothing.hand],
              [this.peer.data.clothing.back, this.peer.data.clothing.mask, this.peer.data.clothing.necklace],
      ...
      this.peer.send(Variant.from("OnConsoleMessage", "`4Ghost mode disabled``. You are back to normal."));
