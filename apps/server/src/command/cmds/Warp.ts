...
export default class Warp extends Command {
  ...
  public async execute(): Promise<void> {
    if (!this.args.length) {
      this.peer.send(Variant.from("OnConsoleMessage", "`4Usage: /warp <world name:door id>"));
      return;
    }
    ...
    if (worldName.length <= 0) {
      this.peer.send(Variant.from("OnConsoleMessage", "`4That world name is empty."));
      return;
    }
    
    if (worldName.match(/\W+|_|EXIT/gi)) {
      this.peer.send(Variant.from("OnConsoleMessage", "`4That world name contains invalid characters."));
      return;
    }

    if (worldName === "EXIT") {
      this.peer.send(Variant.from("OnConsoleMessage", "`4You cannot warp to EXIT."));
      return;
    }

    if (worldName === this.peer.data.world && doorId) {
      const currentWorld = this.peer.currentWorld();
      const door = currentWorld?.data.blocks?.find((b) => b.door && b.door.id === doorId) || 
                   currentWorld?.data.blocks?.find((b) => b.fg === 6);
      
      if (!door) {
        this.peer.send(Variant.from("OnConsoleMessage", "`4Door not found in current world."));
        return;
      }
      
      this.peer.send(Variant.from("OnConsoleMessage", `Warping to door \`2${doorId}\`\` in current world...`));
      ...
      if (currentWorld) {
        currentWorld.every((p) => {
          p.send(
            ...
            Variant.from({ netID: this.peer.data?.netID }, "OnSetPos", [doorX, doorY]),
            Variant.from({ netID: this.peer.data?.netID }, "OnPlayPositioned", "audio/door_open.wav")
          );
        });
      }
    } else {
      if (doorId) {
        this.peer.send(Variant.from("OnConsoleMessage", `Warping to \`2${worldName}:${doorId}\`\`...`));
      } else {
        this.peer.send(Variant.from("OnConsoleMessage", `Warping to \`2${worldName}\`\`...`));
      }
      
      setTimeout(async () => {
        const world = this.base.cache.worlds.get(worldName);
        const door = world?.blocks?.find((b) => b.door && b.door.id === doorId) || 
